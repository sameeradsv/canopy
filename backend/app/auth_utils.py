...
def _validate_cortex_token(db: Session, token: str) -> User | None:
    ...
    if not user:
        # Username may already exist locally (collision) — append suffix to avoid conflict
        base = username
        candidate = base
        suffix = 2
    ...
    # Cache the Cortex token locally so subsequent requests don't hit Cortex Auth Server
    existing_session = db.get(AuthSession, token)
